...
import { baseURL, USERNAME, PASSWORD } from "../config/config";
...
export const Adminlogin = (email, password) => {
  if (email == "karatvalue@gmail.com" && password == "Karatvalue@1234") {
    if (localStorage.getItem("admin")) {
      localStorage.removeItem("admin");
      localStorage.setItem("admin", JSON.stringify({ admin: 1 }));
    } else {
      localStorage.setItem("admin", JSON.stringify({ admin: 1 }));
    }
    return 1;
